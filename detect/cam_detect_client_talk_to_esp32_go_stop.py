...
# ESP32_BASE = "http://192.168.4.1:5000" ESP32 as AP
ESP32_BASE = "http://192.168.0.92:5000"

class GoStopController:
    """
    通用目标触发控制器（target-based）
    - target 连续出现 go_hold_sec 才 GO
    - target 消失持续 stop_hold_sec 才 STOP
    - 状态变化才发请求
    """
    def __init__(self,
                 go_url=f"{ESP32_BASE}/go",
                 stop_url=f"{ESP32_BASE}/stop",
                 go_hold_sec=0.2,
                 stop_hold_sec=0.8,
                 min_send_interval=0.25):
        self.go_url = go_url
        self.stop_url = stop_url
        self.go_hold_sec = go_hold_sec
    ...
    def _http_get(self, url: str):
        try:
            ...
            print("[GoStop] HTTP error:", e)

    def update(self, target_present: bool, now: float):
        if now is None:
            now = time.time()
        ...
        # 发送节流
        if now - self._last_send_t < self.min_send_interval:
            return

        # stop -> go：目标持续出现
        if self._state == "stop":
            if self._first_seen_t is not None and (now - self._first_seen_t) >= self.go_hold_sec:
                self._http_get(self.go_url)
                self._state = "go"
        ...
        # go -> stop：目标持续消失
        if self._state == "go":
            if self._last_seen_t is None or (now - self._last_seen_t) >= self.stop_hold_sec:
                self._http_get(self.stop_url)
                self._state = "stop"
                self._last_send_t = now
                print("[GoStop] -> STOP")
            return
        ...
        if now - last_infer_time > INFER_INTERVAL:
            ...
            if rgb is not None:
                ...
                if dets is not None:
                    ...
                    # talk to esp32 to send go/stop command depending on the person/hand detection
                    TARGET_ID = 0
                    target_present = any((d.get("id") == TARGET_ID and d.get("score", 0.0) >= 0.5) for d in dets)
                    print("[Det] target_present =", target_present, "num_dets =", len(dets))
                    gostop.update(target_present, now=now)
                last_infer_time = now
