...
# DEFAULT_MODEL_PATH = LINE_FOLLOW_DIR / "tflite_models" / "model_int8_uint8_edgetpu.tflite" # first partially usable model
DEFAULT_MODEL_PATH = LINE_FOLLOW_DIR / "tflite_models" / "model_int8_uint8_edgetpu_run_20260607_133850.tflite" # latest model
...
def get_interpreter(model_path: Path, device: str):
    ...
    if CONTEXT["delegate"] is None:
        print(f"line tpu service: loading EdgeTPU delegate on {device} ...")
        CONTEXT["delegate"] = load_edgetpu_delegate(options={"device": device})

    print(f"line tpu service: loading model {model_path}")
...
def start_service(model_path: Path, socket_path: str, device: str, no_reset: bool) -> None:
    if not no_reset:
        reset_tpu_hardware()

    interpreter = get_interpreter(model_path, device)
    print(f"line tpu service: input_size={common.input_size(interpreter)}")
    ...
    print(f"line tpu service started: {socket_path} pid={os.getpid()}")
...
def main() -> None:
    ...
    start_service(args.model, args.socket, args.device, no_reset=not args.reset)
