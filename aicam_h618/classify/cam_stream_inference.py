...
import time
import numpy as np
from tpu_client import remote_tpu_invoke, MODEL_LIST


# ===== TPU inference config =====
MODEL_NAME = 'mobilenet-v1-s'
MODEL_CFG = MODEL_LIST[MODEL_NAME]

TOP_K = 3
THRESHOLD = 0.1

INFER_INTERVAL = 0.2  # 秒，控制推理频率（≈5 FPS）
...
def generate_frames():
    ...
    global last_infer_time, last_results
    camera = get_camera()
    while True:
        with camera_lock:
        ...
        if not success:
            ...
            # ===== TPU inference (client) =====
            rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            img = cv2.resize(rgb, (MODEL_CFG['im_sz'], MODEL_CFG['im_sz']))
            img_np = np.asarray(img, dtype=np.uint8)

            now = time.time()
            if now - last_infer_time > INFER_INTERVAL:
                results, inf_ms = remote_tpu_invoke(
                    MODEL_CFG['fn'],
                    img_np,
                    TOP_K,
                    THRESHOLD
